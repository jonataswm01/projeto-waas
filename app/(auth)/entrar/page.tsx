...
import { userHasOrganization } from "@/app/actions/onboarding";
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (signInError || !data.user) {
        throw new Error(signInError?.message || "Não foi possível entrar.");
      }

      const hasOrg = await userHasOrganization(data.user.id);
      router.push(hasOrg ? "/organizacoes" : "/onboarding");
    } catch (err) {
      setError(
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <button
                type="button"
                className="text-[11px] text-slate-500 hover:text-slate-800 underline underline-offset-4"
              >
                Esqueceu a senha?
              </button>
            </div>
